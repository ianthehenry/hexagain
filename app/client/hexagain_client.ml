...
let parse_model text =
  ...
  let states =
    try [ [%of_sexp: Board_state.t] sexp ] with
    | _ ->
      (try [%of_sexp: Move_abbreviation.t] sexp |> Move_abbreviation.to_board_states with
      | _ -> [%of_sexp: Board_state.t list] sexp)
  in
...
module Route = struct
  ...
  let of_pathname pathname =
    Option.try_with (fun () ->
        match pathname |> String.strip ~drop:(Char.( = ) '/') |> String.split ~on:'/' with
        | [ "game" ] ->
        ...
        | [ "game"; id ] -> Game (ID.Game.of_string id)
        | [ "" ] -> Home
        | _ -> Other)
