...
let direction_between t t' =
  ...
  | -1, 0 -> Some Direction.A
  | 1, 0 -> Some Direction.D
  | 0, 1 -> Some Direction.X
  | -1, 1 -> Some Direction.Z
  | _ -> None
;;
...
let in_direction t direction =
  ...
  | Direction.W -> { row = row - 1; col }
  | Direction.E -> { row = row - 1; col = col + 1 }
  | Direction.A -> { row; col = col - 1 }
  | Direction.D -> { row; col = col + 1 }
  | Direction.Z -> { row = row + 1; col = col - 1 }
  | Direction.X -> { row = row + 1; col }
;;
    ...
    let next_hex =
      ...
      | [ first_choice; second_choice ] ->
        ...
        then (
          ...
          | 2, 3 ->
            ...
            first_choice
          | 3, 2 -> second_choice
          | a, b -> failwithf "unknown edge configuration A (%d, %d)" a b ())
      ...
      | [ first_choice; second_choice; third_choice ] ->
        ...
        then first_choice
          ...
          failwithf !"unknown edge configuration B %{sexp: t list}" possible_next_hexes ()
      | _ ->
